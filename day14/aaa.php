<?php
    ...
    //사용자가 GET방식으로 전달한 값을 변수에 저장한다.
    $nickname= $_GET['nickname'];
    $email= $_GET['email'];

    //실제로는 Database에 저장하는 작업 수행하고 결과를 응답 response-echo
    //DB작업까지 하면 시간이 오래 걸리니 지금은 그냥 받은 데이터를 응답하기
    ...
    echo ("<!DOCTYPE html>
            ...
            <body>
                ...
                <form action='./aaa.php' method='get'>
                    <input type='text' placeholder='닉네임을 입력하세요' name='nickname' value='$nickname'>
                    <input type='text' placeholder='이메일을 입력하세요' name='email' value= '$email'>
                    
                    <input type='submit' value='가입하기'>
                </form>
                <hr>
                <textarea cols='50' rows='5'>$nickname - $email 데이터로 회원가입 했습니다.</textarea>
            </body>
            </html>
    ");
?>